...
2. Check if the system is running: `(resolve 'user/app)` and whether it's bound.
3. If not running, start it from the REPL:
   - `(require '[cjbarre.grain-todo-list :as app])`
   - `(def app (app/start))`
   - The app serves on http://localhost:8080/.
4. Seed any required test data (create + verify accounts and todos via
   `tools/invoke-command!`; see `/verify-ui` for the sign-up + verify-email flow).
5. Verify seed output / projections show the expected counts.

To restart cleanly, stop and start the Integrant system: `(app/stop app)` then
`(def app (app/start))`.
...
- **Bash**: `curl` for webhook testing. For a clean restart, stop/start the system from the
  REPL: `(app/stop app)` then `(def app (app/start))`
