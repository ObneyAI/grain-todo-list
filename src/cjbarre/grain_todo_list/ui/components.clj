     ...
     [:main {:class "min-h-screen bg-base-100 text-base-content"}
      [:div {:class "mx-auto max-w-6xl px-4 py-8"}
       [:header {:class "mb-8 flex flex-col gap-2 border-b border-base-300 pb-6"}
        ...
        [:h1 {:class "text-3xl font-semibold leading-tight"} "Dev UI Gallery"]
        [:p {:class "max-w-2xl text-sm text-base-content/70"}
         "Canonical application surfaces rendered with inert fixture data."]]
       [:div {:class "space-y-10"}
        (gallery-section {:title "Substrate"
                          :status "Shared shell surfaces, alerts, section headers, badges, and empty states."}
                         (inert (action-error))
                         (empty-state "Nothing to show.")
                         (badge-row [{:key :bucket :label "Inbox" :class "badge-outline"}
                                     {:key :status :label "active"}
                                     {:key :due :label "Due 2026-06-03" :class "badge-warning"}
                                     {:key :project :label "Launch reference workflow" :class "badge-secondary"}])
                         (page-section {:title "Nested section specimen"
                                        :count 3
                                        :status "Section actions and counts align with list content."
                                        :action [:button {:class "btn btn-outline btn-sm" :type "button"} "Action"]}
                                       (empty-state "Section content placeholder.")))
        (gallery-section {:title "Forms"
                          :status "Capture and project creation forms in their canonical layout."}
                         (inert (quick-add {:bucket :inbox}))
                         (inert (quick-add {:bucket :next :project-id gallery-project-id}))
                         (inert (project-add)))
        (gallery-section {:title "Tasks"
                          :status "Task cards, summaries, actions, bucket controls, and metadata badges."}
        ...
        (gallery-section {:title "Projects"
                          :status "Project cards, rows, counts, editors, and lifecycle actions."}
                         (inert (project-card active-project))
                         (inert (project-card completed-project))
                         (inert (project-summary-list gallery-projects "No active projects.")))
        (gallery-section {:title "Review"
                          :status "Weekly review controls, progress indicators, and review lists."}
                         ...
                         (inert (review-project-list gallery-review
                                                     ...
                                                     gallery-projects)))
        (gallery-section {:title "Compositions"
                          :status "Representative page-scale compositions assembled from the same components."}
                         (page-section {:title "Inbox"
                                        :count 1
                                        :status "Bucket composition specimen."}
                                       (inert (task-list [active-task] "Nothing here." gallery-projects)))
                         (page-section {:title "Projects"
                                        :count (count gallery-projects)}
